...
description: Use when operating FluxMeld Manager's management API for testing, including health checks, config snapshots, temporary API keys, model mappings, sessions, logs, and cleanup verification.
---

# FluxMeld Management API
...
```bash
FLUXMELD_MGMT_SECRET=mgmt_xxx node skills/fluxmeld-management-api/scripts/management-api.mjs snapshot
FLUXMELD_MGMT_SECRET=mgmt_xxx node skills/fluxmeld-management-api/scripts/management-api.mjs create-api-key --name codex-live-test
FLUXMELD_MGMT_SECRET=mgmt_xxx node skills/fluxmeld-management-api/scripts/management-api.mjs delete-api-key --id key-id
FLUXMELD_MGMT_SECRET=mgmt_xxx node skills/fluxmeld-management-api/scripts/management-api.mjs restore-tool-config --file /private/tmp/toolCallingConfig.json
```
